...
```{r}
clear <- function(row) {
      t <- row['celltype.singResource']
      return(gsub("/", "-", t))
    }
celltypes <- apply(metadata_ref, 1, clear)

names(celltypes) <- colnames(ref[["RNA"]]@counts)
...
```{r}
write.csv(res, "Output/RCTD_results.csv")
```
